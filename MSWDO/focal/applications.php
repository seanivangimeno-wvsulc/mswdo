...
// If ?id=X is specified, fetch details
...
// Search and filter for focal desk
$search_query = isset($_GET['search']) ? trim($_GET['search']) : '';
$status_filter = isset($_GET['status']) ? trim($_GET['status']) : '';

$filtered_apps = [];
foreach ($applications as $app) {
    // Focal officer only manages applications relevant to their program desk.
    // If they are under program ID 1 (AICS), they manage all general AICS claims
    if ($focal_program_id != 1) {
        continue; // Desk constraint safety
    }
...
<head>
    ...
    <title>Desk Review Console - Focal Portal</title>
        ...
        <div class="admin-layout">
            ...
            <main class="admin-panel">
                <div class="admin-header">
                    <div>
                        <h2 style="font-size: 1.5rem; color: var(--primary-dark); margin: 0;">Social Intake Applications Console</h2>
                        <p style="font-size: 0.8rem; color: var(--text-muted); margin-top: 2px;">Review case filings, verify indigent backgrounds, and log recommendations.</p>
                    </div>
                </div>

                <!-- Filters & Search -->
                <div class="card" style="padding: 1.25rem; margin-bottom: 1.5rem;">
                    <form action="/focal/applications.php" method="GET" style="display: flex; gap: 12px; flex-wrap: wrap; align-items: flex-end;">
                        <div class="form-group" style="margin: 0; flex: 1; min-width: 250px;">
                            <label for="search" style="font-size: 0.75rem; font-weight: 700; text-transform: uppercase;">Search Applicant Name or ID</label>
                        ...
                        <div class="form-group" style="margin: 0; width: 180px;">
                            <label for="status" style="font-size: 0.75rem; font-weight: 700; text-transform: uppercase;">Status</label>
                        ...
                        <?php if ($search_query !== '' || $status_filter !== ''): ?>
                            <a href="/focal/applications.php" class="btn btn-outline" style="padding: 10px 16px;">Clear</a>
                        <?php endif; ?>
                    </form>
                </div>
                                ...
                                <?php if (empty($filtered_apps)): ?>
                                    <tr>
                                        <td colspan="6" style="text-align: center; padding: 3rem 1.5rem; color: var(--text-muted);">
                                            No active social welfare applications registered on your desk.
                                        </td>
                                    </tr>
                                <?php else: ?>
                                    <?php foreach ($filtered_apps as $app): ?>
                                        <?php 
                                            $client = $client_map[$app['client_id']] ?? null;
                                            $applicant_name = $client ? ($client['first_name'] . ' ' . $client['last_name']) : 'Guest Client';
                                        ?>
                                        <tr>
                                            ...
                                            <td style="text-align: right; padding-right: 24px;">
                                                <a href="/focal/applications.php?id=<?php echo $app['id']; ?>&search=<?php echo urlencode($search_query); ?>&status=<?php echo urlencode($status_filter); ?>" class="btn btn-primary" style="padding: 6px 12px; font-size: 0.8rem; border-radius: 4px;">
                    ...
                    <div class="modal active" id="review-modal">
                        <div class="modal-content" style="max-width: 700px;">
                            <div class="modal-header">
                                <h3 class="modal-title">Review Case Filing: #<?php echo $view_app['id']; ?></h3>
                                <a href="/focal/applications.php?search=<?php echo urlencode($search_query); ?>&status=<?php echo urlencode($status_filter); ?>" class="modal-close">&times;</a>
                            </div>
                            ...
                            <form action="/php/actions/update_app_status.php" method="POST">
                                ...
                                <div class="modal-body" style="display: flex; flex-direction: column; gap: 1.25rem;">
                                    <div style="background-color: #f8fafc; border-radius: 6px; padding: 1rem; border: 1px solid var(--border); display: grid; grid-template-columns: 1fr 1fr; gap: 10px; font-size: 0.85rem;">
                                        <div>
                                            <p style="color: var(--text-muted); font-size: 0.7rem; text-transform: uppercase; margin: 0;">Citizen Name</p>
                                            <p style="font-weight: 700; color: var(--primary-dark); margin: 0;"><?php echo htmlspecialchars($applicant_name); ?></p>
                                        </div>
                                        <div>
                                            <p style="color: var(--text-muted); font-size: 0.7rem; text-transform: uppercase; margin: 0;">Contact Number</p>
                                        ...
                                        <div style="grid-column: span 2; margin-top: 4px; border-top: 1px solid #e2e8f0; padding-top: 4px;">
                                            <p style="color: var(--text-muted); font-size: 0.7rem; text-transform: uppercase; margin: 0;">Residential Address</p>
                                            <p style="font-weight: 600; color: var(--text); margin: 0;"><?php echo htmlspecialchars($client['address'] ?? 'Tubungan, Iloilo'); ?></p>
                                        </div>
                                    </div>

                                    <div>
                                        <h4 style="font-size: 0.8rem; text-transform: uppercase; color: var(--primary-dark); border-bottom: 1px solid #f1f5f9; padding-bottom: 4px; margin-bottom: 6px;">Requested Assistance</h4>
                                    ...
                                    <div>
                                        <h4 style="font-size: 0.8rem; text-transform: uppercase; color: var(--primary-dark); border-bottom: 1px solid #f1f5f9; padding-bottom: 4px; margin-bottom: 6px;">Case Circumstances / Narrative</h4>
                                        <p style="font-size: 0.85rem; color: var(--text-muted); line-height: 1.5; background-color: #fffbeb; padding: 10px; border-radius: 4px; border-left: 3px solid var(--accent); margin: 0; white-space: pre-line;">
                                            <?php echo htmlspecialchars($view_app['findings'] ?: 'No details filed.'); ?>
                                        </p>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <?php if (empty($view_famcom)): ?>
                                            <p style="font-size: 0.8rem; color: var(--text-muted); font-style: italic;">No family members listed.</p>
                                        <?php else: ?>
                                    ...
                                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                                        <div>
                                            <h4 style="font-size: 0.8rem; text-transform: uppercase; color: var(--primary-dark); border-bottom: 1px solid #f1f5f9; padding-bottom: 4px; margin-bottom: 4px;">Digital Signature Affirmation</h4>
                                        ...
                                        <div>
                                            <h4 style="font-size: 0.8rem; text-transform: uppercase; color: var(--primary-dark); border-bottom: 1px solid #f1f5f9; padding-bottom: 4px; margin-bottom: 4px;">File Attachment</h4>
                                            <?php if ($filename_str): ?>
                                                <a href="/uploads/<?php echo htmlspecialchars($filename_str); ?>" target="_blank" class="btn btn-outline" style="padding: 4px 10px; font-size: 0.75rem; border-radius: 4px; display: inline-flex; align-items: center; gap: 4px; margin-top: 4px;">
                                                    <span class="material-symbols-outlined" style="font-size: 14px;">visibility</span> Inspect Attachment File
                                                </a>
                                            <?php else: ?>
                                                <p style="font-size: 0.8rem; color: var(--text-muted); font-style: italic; margin-top: 4px;">No file uploaded.</p>
                                            <?php endif; ?>
                                        </div>
                                    </div>

                                    <!-- Action decision inputs -->
                                    <div style="border-top: 1px solid #e2e8f0; padding-top: 1rem; display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                                        <div class="form-group" style="margin: 0;">
                                            <label for="status-decision"><strong>Action Intake Decision</strong></label>
                                            <select name="status" id="status-decision" class="form-control" required style="background: #fffbeb; border-color: var(--accent); font-weight: 600;">
                                                ...
                                                <option value="Rejected" <?php echo $view_app['status'] == 'Rejected' ? 'selected' : ''; ?>>Reject / Disapprove Claims</option>
                                            </select>
                                        </div>
                                        <div class="form-group" style="margin: 0;">
                                            <label for="prepared_by"><strong>Officer-In-Charge Signature</strong></label>
                                            <input type="text" name="prepared_by" id="prepared_by" class="form-control" value="<?php echo htmlspecialchars($view_app['prepared_by'] ?: $_SESSION['focal_name']); ?>" required>
                                        </div>
                                    </div>

                                    <div class="form-group" style="margin: 0;">
                                        <label for="recommendation"><strong>Action Recommendation Remarks</strong></label>
                                        <textarea name="recommendation" id="recommendation" class="form-control" rows="3" placeholder="State reasons or next workflow steps..." required><?php echo htmlspecialchars($view_app['recommendation'] ?? ''); ?></textarea>
                                    </div>
                                </div>

                                <div class="modal-footer">
                                    <a href="/focal/applications.php?search=<?php echo urlencode($search_query); ?>&status=<?php echo urlencode($status_filter); ?>" class="btn btn-outline">Close Reviewer</a>
                                    <button type="submit" class="btn btn-primary">
                                        <span class="material-symbols-outlined" style="font-size: 16px;">save</span> Save Updates
                                    </button>
                                </div>
                            </form>
                        </div>
